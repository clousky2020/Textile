class PurchaseSupplier < ApplicationRecord
  ...
  def calcu_total_payment_required
    ...
    if self.check_money_time && self.check_money
      ...
      # 再加包含运费的单子里的运费
      need_pay_money += purchase_orders.where(our_freight: false).collect(&:freight).sum
    ...
    is_return_money += purchase_orders.where(is_return: true, our_freight: false).collect(&:freight).sum
    # 更新
