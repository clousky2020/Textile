...
describe "purchase_orders" do
  ...
  describe "采购订单列表的查询" do
    let!(:purchase_orders) {create_list(:purchase_order, 10, user: user, purchase_supplier: purchase_supplier, material: material, repo: repo)}
    before do
      visit purchase_orders_path
    end
    ...
    scenario "未作废订单-已作废订单" do
      ...
      purchase_orders.each do |purchase_order|
        expect(page).to have_content(purchase_order.order_id)
        purchase_order.order_declare_invalid(user)
      end

      select '未作废订单', from: 'type'
      click_button '搜索'
      purchase_orders.each do |purchase_order|
        expect(page).not_to have_content(purchase_order.order_id)
      end
    ...
    describe "特定名称" do
      ...
      scenario "原料名字" do
        fill_in 'search', with: purchase_orders[0].material.name
        select '未作废订单', from: 'type'
        click_button '搜索'
        expect(page).to have_content(purchase_orders[0].material.name)
      end
      scenario "产品型号" do
        fill_in 'search', with: purchase_orders[0].material.specification
        select '未作废订单', from: 'type'
        click_button '搜索'
        expect(page).to have_content(purchase_orders[0].material.specification)
      end
      scenario "供应商名字" do
        fill_in 'search', with: purchase_orders[0].purchase_supplier.name
        select '未作废订单', from: 'type'
        click_button '搜索'
        expect(page).to have_content(purchase_orders[0].purchase_supplier.name)
      end
      scenario "订单号" do
        fill_in 'search', with: purchase_orders[0].order_id
        select '未作废订单', from: 'type'
        click_button '搜索'
        expect(page).to have_content(purchase_orders[0].order_id)
      end
